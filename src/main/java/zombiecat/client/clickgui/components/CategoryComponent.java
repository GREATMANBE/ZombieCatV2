...
public class CategoryComponent {
    ...
    public void rf(FontRenderer renderer) {
        ...
        if (GuiModule.categoryBackground.getValue()) {
            BooleanComponent.renderMain(
                (float)(this.x - 2),
                (float)this.y,
                (float)(this.x + this.width + 2),
                (float)(this.y + this.bh + 3),
                -1
            );
        }
        ...
        renderer.drawString(
            ...
            (float)(this.y + 4),
            Color.getHSBColor(
                (float)(System.currentTimeMillis() % (7500L / (long)this.chromaSpeed)) / (7500.0F / (float)this.chromaSpeed),
                1.0F,
                1.0F
            ).getRGB(),
            false
        ...
        if (!this.n4m) {
            ...
            renderer.drawString(
                this.categoryOpened ? "-" : "+",
                (float)((double)this.x + this.marginX),
                (float)((double)this.y + this.marginY),
                Color.white.getRGB(),
                false
            );
            GL11.glPopMatrix();
