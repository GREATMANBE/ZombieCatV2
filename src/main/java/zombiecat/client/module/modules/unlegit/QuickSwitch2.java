...
public class QuickSwitch2 extends Module {
        ...
        if (this.isOn() && mc.thePlayer != null && mc.theWorld != null) {
            ...
            if (cSlot == 2 && s2.getValue()) {
                ...
                for (int slot : toggledSlots) {
                    if (this.isToggled(slot)) { doSwap(slot); return; }
                }
            }
            ...
            if (cSlot == 3 && s3.getValue()) {
                ...
                for (int slotx : toggledSlots) {
                    if (this.isToggled(slotx)) { doSwap(slotx); return; }
                }
            }
            ...
            if (cSlot == 4 && s4.getValue()) {
                ...
                for (int slotxx : toggledSlots) {
                    if (this.isToggled(slotxx)) { doSwap(slotxx); return; }
                }
            }
            ...
            if (anySlot.getValue() && cSlot != 1) {
                ...
                for (int slotxxxxxxx : toggledSlots) {
                    if (this.isToggled(slotxxxxxxx)) { doSwap(slotxxxxxxx); return; }
                }
            }
        }
    }

    // --- Rapid NoReload implementation ---
    @SubscribeEvent
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (mc.gameSettings.keyBindUseItem.isKeyDown()) {
            // Loop multiple times per tick for rapid swaps
            for (int i = 0; i < 5; i++) { // adjust the number for more/less swaps
                doNoReloadNow();
            }
        }
    }
    ...
    private void clickMagicSlot() {
        originalItem = mc.thePlayer.getHeldItem().copy();
        windowId = mc.thePlayer.openContainer.windowId;

        mc.getNetHandler().addToSendQueue(
                new C0EPacketClickWindow(windowId, 41, 0, 0, originalItem, actionNumber++)
        );
    }
    ...
    private void swapItemBack() {
        ...
        for (int i = 9; i < container.inventorySlots.size(); i++) {
            ...
            if (stackInSlot != null && ItemStack.areItemsEqual(stackInSlot, originalItem)) {
                mc.getNetHandler().addToSendQueue(
                        new C0EPacketClickWindow(windowId, i, 0, 0, stackInSlot, actionNumber++)
                );
                break;
            }
        }
