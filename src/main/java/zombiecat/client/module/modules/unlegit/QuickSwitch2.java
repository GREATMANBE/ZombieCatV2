    ...
    public void a(TickEvent.RenderTickEvent ev) {
        if (ev.phase == TickEvent.Phase.END && Utils.Player.isPlayerInGame()) {
            ...
            mc.fontRendererObj.drawStringWithShadow(
                "Preset:1 " + "Slot2:" + s2.getValue() + " Slot3:" + s3.getValue() + " Slot4:" + s4.getValue() + " Delay:" + delay.getValue(),
                10, 10, 0xFFFFFF
            );
        }
    }
    ...
    public static void doSwap(int slot) {
        ...
        for (int i = 0; i < mc.gameSettings.keyBindsHotbar.length; i++) {
            if (i == slot - 1) {
                ...
                // --- Extra: silently right-click hotbar slot 6 (index 41) ---
                if (mc.thePlayer != null && mc.playerController != null) {
                    mc.playerController.windowClick(
                        mc.thePlayer.inventoryContainer.windowId, // inventory container
                        41,                                       // 6th hotbar slot
                        1,                                        // right click
                        0,                                        // normal click
                        mc.thePlayer
