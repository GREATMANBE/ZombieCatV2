...
public class Aimbot extends Module {
    ...
    public static SliderSetting aimYOffset; // NEW slider for Y aim offset
    ...
    public Aimbot() {
        ...
        this.registerSetting(aimYOffset = new SliderSetting("Aim Y Offset", 1.5, 0.0, 3.0, 0.1)); // default 1.5 blocks
    }
        ...
        if (Utils.Player.isPlayerInGame()) {
            for (Entity entity : mc.theWorld.loadedEntityList) {
                if (entity instanceof EntityLivingBase
                    ...
                    // Calculate target position using adjustable Y offset
                    Vec3 basePos = entity.getPositionVector().add(offset);
                    Vec3 aimPos = new Vec3(basePos.xCoord, entity.getPositionVector().yCoord + aimYOffset.getValue(), basePos.zCoord);

                    double distance = fovDistance(aimPos);
                    if (distance < dis && canWallShot(mc.thePlayer.getPositionEyes(1), aimPos)) {
                        dis = distance;
                        target = aimPos;
                    }
                }
